...
const SentIcon = () => (
  <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
    <path d="M13.5 2.5L7 9M13.5 2.5L9 14l-2-5-5-2 11.5-4.5z" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
)
const DraftsIcon = () => (
  <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
    <path d="M10 2H4a1 1 0 00-1 1v10a1 1 0 001 1h8a1 1 0 001-1V6l-3-4z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round"/>
    <path d="M10 2v4h4M6 9h4M6 11.5h2" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/>
  </svg>
)
const TrashIcon = () => (
  <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
...
export function Sidebar() {
  ...
  const handleFolderClick = async (accountId: string, folderPath: string) => {
    ...
    try {
      const data = await emailsApi.list(accountId, folderPath)
      setEmails(data)
    } catch (err) {
      console.error(err)
    } finally {
  ...
  return (
    <aside className="flex flex-col h-full bg-[#161b22] border-r border-[#30363d] w-52 flex-shrink-0">
      {/* Compose */}
      <div className="p-3">
        <button
          onClick={() => openCompose()}
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2">
        {accounts.length === 0 ? (
          <div className="px-3 py-8 text-center">
            ...
            <p className="text-[#8b949e] text-xs mb-2">No accounts yet</p>
              ...
              <div key={account.id} className="mb-1">
                ...
                <div
                  className={`flex items-center gap-2 px-2 py-2 rounded-md cursor-pointer group transition-colors
                    ${isActive ? 'bg-[#21262d]' : 'hover:bg-[#1c2128]'}`}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[11px] font-semibold text-[#e6edf3] truncate">{account.name || account.email}</div>
                    <div className="text-[10px] text-[#8b949e] truncate">{account.email}</div>
                  </div>
                  <button
                    onClick={(e) => handleDeleteAccount(account.id, e)}
                    className="opacity-0 group-hover:opacity-100 text-[#484f58] hover:text-[#f85149] p-0.5 rounded transition-all"
                  ...
                  <div className="mt-0.5 ml-1">
                    {accountFolders.map(folder => {
                      ...
                      return (
                        ...
                        >
                          <span className={isActiveFolder ? 'text-[#f59e0b]' : 'text-[#484f58]'}>
                            <Icon />
                          </span>
      ...
      <div className="p-3 border-t border-[#30363d]">
        <button
          ...
          className="w-full flex items-center gap-2 px-2 py-2 text-xs text-[#8b949e] hover:text-[#f59e0b] hover:bg-[#1c2128] rounded-md transition-colors"
        >
