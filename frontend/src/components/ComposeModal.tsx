...
const inputCls = 'flex-1 text-sm bg-transparent text-[#e6edf3] placeholder-[#484f58] focus:outline-none'
const labelCls = 'text-[11px] text-[#484f58] w-12 flex-shrink-0'
const rowCls   = 'flex items-center gap-2 px-4 py-2.5 border-b border-[#30363d]'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-end p-4 pointer-events-none">
      <div className="flex gap-3 pointer-events-auto">
        ...
        {/* AI Panel */}
        {showAiPanel && (
          <div className="w-72 bg-[#161b22] border border-[#30363d] rounded-xl shadow-2xl flex flex-col" style={{ height: '520px' }}>
            {/* AI Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#30363d] rounded-t-xl bg-[#1c2128]">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-sm text-[#e6edf3]">Claude AI</span>
              </div>
              <button onClick={() => setShowAiPanel(false)} className="text-[#484f58] hover:text-[#e6edf3] transition-colors">
            ...
            <div className="p-3 border-b border-[#30363d]">
              <p className="text-[10px] text-[#484f58] mb-2 uppercase tracking-wide font-semibold">Mode</p>
                ...
                {AI_MODES.map(mode => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-0.5 p-1.5 rounded-md text-[10px] transition-colors border
                      ${aiMode === mode.value
                        ? 'bg-[rgba(139,92,246,0.15)] text-violet-400 border-violet-500/30'
              ...
              {aiMode === 'custom' && (
                <textarea
                  ...
                  className="mt-2 w-full p-2 text-xs bg-[#21262d] border border-[#30363d] text-[#e6edf3] placeholder-[#484f58] rounded-md resize-none focus:outline-none focus:border-violet-500/50"
                  rows={2}
                />
              )}
              ...
              <button
                ...
                className={`mt-2 w-full py-2 rounded-md text-xs font-semibold transition-colors
                  ${isAiLoading
                    ? 'bg-[#f85149]/20 border border-[#f85149]/40 text-[#f85149] hover:bg-[#f85149]/30'
            ...
            <div className="flex-1 overflow-y-auto p-3">
              {isAiLoading && !aiSuggestion && (
                <div className="flex items-center gap-2 text-violet-400 text-xs">
                  ...
                  <span className="flex gap-1">
                    {[0, 1, 2].map(i => (
                      <span key={i} className="ai-loading-dot w-1 h-1 rounded-full bg-violet-400 inline-block" />
                    ))}
                  </span>
                </div>
              )}
              {aiSuggestion && (
                <div>
                  <div className="text-[10px] text-[#484f58] mb-1.5 font-semibold uppercase tracking-wide">
                    Suggestion {isAiLoading && <span className="text-violet-400 ml-1 normal-case">streaming…</span>}
                  </div>
                  <div className="text-xs text-[#c9d1d9] bg-[#21262d] border border-[#30363d] rounded-md p-3 whitespace-pre-wrap leading-relaxed">
                    {aiSuggestion}
              ...
              {!aiSuggestion && !isAiLoading && (
                <div className="text-[11px] text-[#484f58] text-center py-4 leading-relaxed">
            ...
            {aiDone && aiSuggestion && (
              <div className="p-3 border-t border-[#30363d] flex gap-2">
                ...
                <button
                  onClick={() => { setAiSuggestion(''); setAiDone(false) }}
                  className="px-3 py-2 text-[#484f58] hover:text-[#e6edf3] hover:bg-[#21262d] rounded-md text-xs transition-colors"
                >
                  ✕
                </button>
              </div>
        ...
        {/* Compose window */}
        <div className="w-[540px] bg-[#161b22] border border-[#30363d] rounded-xl shadow-2xl flex flex-col" style={{ height: '520px' }}>
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 bg-[#1c2128] border-b border-[#30363d] rounded-t-xl">
            <span className="text-[#e6edf3] font-semibold text-sm">
              {composeData.replyTo ? 'Reply' : 'New Message'}
            </span>
            <div className="flex items-center gap-2">
              <button onClick={closeCompose} className="text-[#484f58] hover:text-[#e6edf3] transition-colors p-0.5" title="Minimize">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  <path d="M2 7h10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                </svg>
              </button>
              <button onClick={closeCompose} className="text-[#484f58] hover:text-[#f85149] transition-colors p-0.5" title="Close">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  <path d="M2 2l10 10M12 2L2 12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                </svg>
              </button>
          ...
          <div className="flex flex-col flex-1 min-h-0">
            {accounts.length > 1 && (
              <div className={rowCls}>
                ...
                <select
                  ...
                  className="flex-1 text-xs bg-transparent text-[#e6edf3] focus:outline-none"
                >
                  {accounts.map(a => <option key={a.id} value={a.id} className="bg-[#161b22]">{a.email}</option>)}
                </select>
              </div>
            )}
            ...
            <div className={rowCls}>
              ...
              <input type="email" value={to} onChange={e => setTo(e.target.value)}
                placeholder="recipient@example.com" className={inputCls} multiple />
              ...
              <>
                <div className={rowCls}>
                  ...
                  <input type="text" value={cc} onChange={e => setCc(e.target.value)}
                    placeholder="cc@example.com" className={inputCls} />
                </div>
                <div className={rowCls}>
                  <span className={labelCls}>Bcc</span>
                  <input type="text" value={bcc} onChange={e => setBcc(e.target.value)}
                    placeholder="bcc@example.com" className={inputCls} />
                </div>
              </>
            )}
            ...
            <div className={rowCls}>
              ...
              <input type="text" value={subject} onChange={e => setSubject(e.target.value)}
                placeholder="Email subject" className={`${inputCls} font-medium`} />
            </div>

            {composeData.replyTo && (
              <div className="px-4 py-2 border-b border-[#30363d] bg-[#1c2128]">
                <div className="text-[10px] text-[#484f58]">
                  Replying to <span className="text-[#8b949e]">{composeData.replyTo.from}</span>
                </div>
              </div>
            )}
            ...
            <textarea
              ...
              className="flex-1 px-4 py-3 text-sm bg-transparent text-[#c9d1d9] placeholder-[#484f58] focus:outline-none resize-none font-sans leading-relaxed"
            />

            {/* Bottom toolbar */}
            <div className="flex items-center gap-2 px-4 py-2.5 border-t border-[#30363d] bg-[#1c2128] rounded-b-xl">
              <button
                onClick={handleSend}
                disabled={isSending}
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 rounded-md text-xs font-semibold transition-all
                  ${showAiPanel
                    ...
                    : 'bg-gradient-to-r from-violet-600/20 to-blue-600/20 text-[#8b949e] border border-[#30363d] hover:text-[#e6edf3] hover:border-violet-500/30'
                  }`}
              >
                ✦ AI Assist
              </button>
              ...
              <button
                ...
                className="p-2 text-[#484f58] hover:text-[#f85149] hover:bg-[#21262d] rounded-md transition-colors"
                title="Discard"
              >
                <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
                  <path d="M2.5 4.5h11M6 4.5V3h4v1.5M4 4.5l.7 8.5h6.6L12 4.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </button>
            </div>
          </div>
